...
function imgui.OnDrawFrame()
    ...
    if frames.prmwindow.v then
    	imgui.SetNextWindowPos(imgui.ImVec2(resX / 2 , resY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
        imgui.SetNextWindowSize(imgui.ImVec2(300, 410), imgui.Cond.FirstUseEver)
    	imgui.Begin('PR Manager (OS '..thisScript().version..')##prmenu', frames.prmwindow, imgui.WindowFlags.NoResize)
        	if checkboxes.prmanager.v then
	        	if imgui.Checkbox(u8'Реклама в VIP CHAT', checkboxes.vr1) then cfg.settings.vr1 = checkboxes.vr1.v end
				...
				if imgui.Checkbox(u8'Реклама в VIP CHAT (Доп. строка)', checkboxes.vr2) then cfg.settings.vr2 = checkboxes.vr2.v end
				if checkboxes.vr2.v then
					imgui.Text(u8'Сообщение: ')
					imgui.SameLine()
					if imgui.InputTextWithHint(u8"##vr2", u8"Работает БК Лыткарино №56!", buffers.vrmsg2) then cfg.settings.vrmsg2 = buffers.vrmsg2.v end
					end
				if imgui.Checkbox(u8'Реклама в FAMILY CHAT', checkboxes.fam) then cfg.settings.fam = checkboxes.fam.v end
				...
				if imgui.Checkbox(u8'Реклама в FAMILY CHAT (Доп. строка)', checkboxes.fam1) then cfg.settings.fam1 = checkboxes.fam1.v end
				if checkboxes.fam1.v then
					imgui.Text(u8'Сообщение: ')
					imgui.SameLine()
					if imgui.InputTextWithHint(u8"##fammsg1", u8"Работает БК Эдово №57!", buffers.fammsg1) then cfg.settings.fammsg1 = buffers.fammsg1.v end
				end
				if imgui.Checkbox(u8'Реклама в ALLIANCE CHAT ', checkboxes.al) then cfg.settings.al = checkboxes.al.v end
				...
				if imgui.Checkbox(u8'Реклама в ALLIANCE CHAT (Доп. строка)', checkboxes.al1) then cfg.settings.al1 = checkboxes.al1.v end
				if checkboxes.al1.v then
					imgui.Text(u8'Сообщение: ')
					imgui.SameLine()
					if imgui.InputTextWithHint(u8"##almsg1", u8"Работает БК Лыткарино №56!", buffers.almsg1) then cfg.settings.almsg1 = buffers.almsg1.v end
					end
				imgui.Separator()
		    ...
		    if imgui.Button(u8'Сохранить', imgui.ImVec2(295, 20)) then
		        save()
		        msg('Все настройки сохранены.')
		    end
...
function piar()
	lua_thread.create(function()
			...
			if pronoroff and checkboxes.fam1.v then
				send('/fam '..u8:decode(buffers.fammsg1.v))
			end
			wait(7000)
			...
			if pronoroff and checkboxes.al1.v then
				send('/al '..u8:decode(buffers.almsg1.v))
			end
